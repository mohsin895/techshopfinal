            ...
            ?>
                        ...
                        <div data-kt-menu-trigger="click" class="menu-item menu-accordion">
                            <span class="menu-link">
                                ...
                                <span class="menu-section text-muted text-uppercase fs-8 ls-1">Account System</span>

                                <span class="menu-arrow"></span>
                            </span>
                            <?php
                    $role = DB::table('roles')->find(Auth::guard('admin')->user()->role_id);
                    $creadit_view_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                    ['permissions.name', 'creadit_view'],
                    ['role_id', $role->id]
                    ])->first();
                    $debit_index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                    ['permissions.name', 'debit_index'],
                    ['role_id', $role->id]
                    ])->first();


                    ?>
                            @if($creadit_view_permission_active)
                            ...
                            @endif
                            @if($debit_index_permission_active)
                            <div class="menu-sub menu-sub-accordion menu-active-bg">
                                <div class="menu-item">
                                    <a href="{{route('admin.debit')}}">
                                        <span class="menu-link">
                                            <span class="menu-icon">
                                                <!--begin::Svg Icon | path: icons/duotune/communication/com013.svg-->
                                                <span class="svg-icon svg-icon-2">
